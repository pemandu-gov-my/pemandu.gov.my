...
<li class="homebutton"><a  href="<?php echo  bloginfo('siteurl') ?>"  style="margin-left:1px; font-size:12px; width: 40px; padding-top: 10px; margin-top:0; height: 20px;">HOME</a></li>
 ...
 <object width="170" height="170"><param name="movie" value="http://www.youtube.com/v/kqUrdyUGWhU?fs=1&amp;hl=en_US"></param><param name="allowFullScreen" value="true"></param><param name="allowscriptaccess" value="always"></param><embed src="http://www.youtube.com/v/kqUrdyUGWhU?fs=1&amp;hl=en_US" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="170" height="170"></embed></object>
...
<a href="http://www.pemandu.gov.my/gtp/?feedback=form"><img src="<?php bloginfo('template_directory'); ?>/images/tell-us.png" /></a></div>
			...
			<div id="content-container">
					...
					<?php
						...
						$myArray = $wpdb->get_results( "SELECT `id`, `time`, `video_link`, `file_name`, `status`  FROM pm_videos where status=1 order by id desc $limit",ARRAY_A);
				...
				?>
					...
					<table width="503" id="news_table" style="margin-left:5px; margin-top:-14px;">

							<thead>

									<th width="75%" style="text-align: left; font-size:14px;"> REPORTS </th>

									<th style="text-align: center; font-size:14px;">DOWNLOAD </th>
				...
				?>
					...
					<table width="503" id="news_table" style="margin-left:5px; margin-top:-14px;">
							...
							<thead>
									...
									<th width="75%" style="text-align: left; font-size:14px;"> PRESS RELEASES </th>
							...
							<?php 
									...
									$video_arr = $wpdb->get_results( "SELECT *  FROM pm_pressrelease where status='1' ",ARRAY_A);
									...
									$p->target(get_option('siteurl')."?page_id=689");
									...
									$report_data = $wpdb->get_results("SELECT `id`, `time`, `press_title`, `file_name`, `status`  FROM pm_pressrelease where status=1  order by id desc $limit",ARRAY_A);
									...
									<tr class="tr1" style="background-color:<?php echo $bgcolor; ?>;" >
										...
										<td><?php echo $links_arrr['press_title'];?></td>
					...
					<table width="503" id="news_table" style="margin-left:5px; margin-top:-14px;">
							<thead>
									...
									<th width="75%" style="text-align: left; font-size:14px;"> NEWS HEADLINES</th>
							...
							<?php 
									...
									$news_data = $wpdb->get_results( "SELECT  `ID`,`post_date`,`post_content`,`post_title`  FROM pm_posts WHERE `post_type`='post' and `post_status`='publish' order by post_date desc $limit",ARRAY_A);
					...
					<div class="media_content_section" style="width:100%;">
						...
						<form id="contact-pemandu-form" name="contact-pemandu-form" method="post" action="">
							...
							<p style='margin-bottom:4px;'>Your feedback / suggestions:</p>
							...
							<h3 style='margin-top:8px;'>Additional Information (Optional)</h3>

							<p style='margin-bottom:4px; margin-top:10px;'>Name:</p>

							<input type="text" name="name_field" id="name_field" size="38" />

							<br />

							<p style='margin-bottom:4px; margin-top:10px;'>Age:</p>

							<input type="text" name="age_field" id="age_field" size="38" />

							<p style='margin-bottom:4px; margin-top:10px;'>Location:</p>
							...
							<br><p>You can also share your thoughts directly to us at: <a href="mailto:feedback.gtp@pemandu.gov.my" style ="color:#1B75BB; text-decoration:none;">feedback.gtp@pemandu.gov.my</a></p>
							...
							<div id="response" style="color:#996600; font-size:14px; text-align:right; margin-bottom:5px;"><?php if($case==2){echo "Message Successfully Sent. Thank You!";} ?></div>
