...
var PLUGIN_PATH = SITE_HOME_PATH+'wp-content/plugins/slider/';
...
function validate_add_slider()
{
	...
	url: PLUGIN_PATH+'process/add_slider.php' 
	};
...
function delete_slider(field_id)
{
...
function validate_update_slider()
{
	
	$('#update_slider_frm').unbind('submit');
	var options = {
	target: '#error_div',	
	beforeSubmit: show_mem_slider_Request, 	
	success: show_mem_slider_Response, 
	url: PLUGIN_PATH+'process/update_slider_field.php'
	};
	$('#update_slider_frm').submit(function() {
		$(this).ajaxSubmit(options);
		return false;
	});
}

function show_mem_slider_Request(formData, jqForm, options) 
{
	var queryString = $.param(formData);
...
function show_mem_slider_Response(responseText, statusText) 
{
