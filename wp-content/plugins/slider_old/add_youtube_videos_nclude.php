<script type="text/javascript" src="<?php echo bloginfo('wpurl').'/wp-content/plugins/add_youtube_videos/'; ?>js/jquery-1.3.2.min.js"></script>
<script type="text/javascript" src="<?php echo bloginfo('wpurl').'/wp-content/plugins/add_youtube_videos/'; ?>js/jquery.form.js"></script>
<script type="text/javascript" src="<?php echo bloginfo('wpurl').'/wp-content/plugins/add_youtube_videos/'; ?>js/ajaxfunctions.js"></script>
<link type="text/css" rel="stylesheet" href="<?php echo bloginfo('wpurl').'/wp-content/plugins/add_youtube_videos/'; ?>css/mystyle.css" />
<script type="text/javascript" src="<?php echo bloginfo('wpurl').'/wp-content/plugins/add_youtube_videos/'; ?>js/colorbox/jquery.colorbox.js"></script>
<link rel="stylesheet" type="text/css" href="<?php echo bloginfo('wpurl').'/wp-content/plugins/add_youtube_videos/'; ?>css/colorbox/colorbox.css" />
<script>
...
display:none;
	...
	<div class="wrap">  
    <?php    echo "<h2>" . __( 'Add Youtube videos', 'oscimp_trdom' ) . "</h2><hr>"; ?>  
	<?php if($error_msg !=""){?>
	<div class="error_msg">
...
<div class="add_mem_fields">
     <form name="add_vido_field_frm" id="add_vido_field_frm" method="post" action="<?php echo str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">  
         ...
         <p class="p_height">
		 	<span class="span_label">Youtube Link</span>
			<span class="span_field"><input type="text" name="video_link" id="video_link" value="<?php echo $_POST['video_link'];?>" size="80"><?php _e(" "); ?></span>
		 </p>
		 <p class="submit" style="text-align:left; margin-left:190px;">  
         <input type="submit" name="Submit" value="<?php _e('Add', 'oscimp_trdom' ) ?>" onclick="return validate_add__youtube_videos();" />  
         </p>  
     </form> 
   </div> 
</div>
	...
	<table class="widefat fixed" cellspacing="0">
		...
		<tbody id="users" class="list:user user-list">
			<?php
				global $wpdb;
				$form_fields_arr = $wpdb->get_results( "SELECT *  FROM pm_videos ",ARRAY_A);
		...
		<tr id="field-<?php echo $form_fields_record_arr['id']; ?>" onmouseover="change_tr_color(this);" onmouseout="back_to_original(this);" bgcolor="<?php echo $bg_color;?>">
			...
			<td class="username column-username"><?php echo $form_fields_record_arr['video_link']; ?> 
				<strong>
